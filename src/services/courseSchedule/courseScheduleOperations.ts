...
import { courseScheduleStatusService } from './courseScheduleStatusService';

export const courseScheduleOperations = {
  ...
  async createSchedule(scheduleData: CourseScheduleFormData): Promise<CourseSchedule> {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        status: courseScheduleStatusService.calculateStatus(scheduleData.start_date, dateCalculation.end_date)
      })
      .select()
      .single();
  ...
  async updateSchedule(id: string, scheduleData: Partial<CourseScheduleFormData>): Promise<CourseSchedule> {
    ...
    if (scheduleData.start_date || scheduleData.course_offering_id) {
      ...
      if (durationDays > 0) {
        ...
        updateData.status = courseScheduleStatusService.calculateStatus(startDate, dateCalculation.end_date);
      }
    }
